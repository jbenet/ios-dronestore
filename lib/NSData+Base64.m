	...
	size_t *outputLength)
{
    ...
    memset(accumulated, 0, BASE64_UNIT_SIZE);
...
//
// Creates an NSData object containing the base64 encoded representation of
// the string 'aString'
//
// Parameters:
...
//
// returns the autoreleased NSData representation of the base64 string
+ (NSData *)base64DataFromString:(NSString *)aString
{
	NSData *data = [aString dataUsingEncoding:NSASCIIStringEncoding];
	size_t outputLength;
	void *outputBuffer = NewBase64Encode([data bytes], [data length], false, &outputLength);
	NSData *result = [NSData dataWithBytes:outputBuffer length:outputLength];
	free(outputBuffer);
	return result;
}
...
// base64DecodedString
//
// Creates an NSString object that contains the decoded version of the
// receiver's encoded data. Lines are NOT broken at 64 characters long.
//
// returns an autoreleased NSString being the decoded representation of the
//	receiver.
//
